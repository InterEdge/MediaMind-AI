...
const edgeCode = ts.transpileModule(edgeSource, { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 } }).outputText;
function edgeFixture({ docs = [{ id: documentId, title: "Campaign", extracted_text: "Verified campaign results." }], providerContent = JSON.stringify(result), denied = false, providerTimeout = false } = {}) {
  ...
  const authHelpers = {
    authenticateEdgeRequest: async () => ({ serviceClient: { from: () => query } }),
  ...
  }, {}, { serve: (fn) => { handler = fn; } }, async (_url, options) => {
...
test("draft save succeeds with a warning when secondary activity tracking fails", async () => {
  for (const throws of [false, true]) {
    ...
    new Function("require", "exports", serviceCode)((name) => {
